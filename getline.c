 ...
 * Return: NULL if memory allocation fails else
...
static void *_realloc(void *old, size_t old_size, size_t new_size)
{
	...
	if (old)
	{
		if (new_size)
		{
			...
			if (new)
			{
				_memcpy(new, old_size < new_size ? old_size : new_size);
				free(old);
			}
		}
		else
...
/**
 * _getline_next - read input of input
 * @buf: pointer to the static buffer
 * @line: address of a pointer tothe line
 * @size:address of a pointer to the line size
 * @n:number of characters to copy from the buffer
 * Return: NULL if memory allocation fails,else return
 * pointer to the line of input
 */
